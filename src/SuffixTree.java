...
/**
 * 后缀树的Ukkonen算法实现，建树时间为O（N）
 * 原理解释 http://www.oschina.net/translate/ukkonens-suffix-tree-algorithm-in-plain-english
 *
 * 论文中使用边来保存字符信息，此处实现使用节点保存字符信息，没有差别。
 * 同时树的结构通过子节点和兄弟节点保存，即遍历一个节点的子节点，需先找到其直接子节点，然后通过该
 * 子节点访问其兄弟节点找到全部子节点
 * 实现理解：http://vickyqi.com/2015/11/27/%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E7%B3%BB%E5%88%97%E2%80%94%E2%80%94%E5%90%8E%E7%BC%80%E6%A0%91Java%E5%AE%9E%E7%8E%B0%E4%BB%A3%E7%A0%81/
 *
 * 后缀树的典型用途：
 * (1). 查找字符串o是否在字符串S中。
 * 方案：用S构造后缀树，按在trie中搜索字串的方法搜索o即可。
 * 原理：若o在S中，则o必然是S的某个后缀的前缀。
 * 例如S: leconte，查找o: con是否在S中,则o(con)必然是S(leconte)的后缀之一conte的前缀.有了这个前提，
 * 采用trie搜索的方法就不难理解了。
 * (2). 指定字符串T在字符串S中的重复次数。
 ...
 * (5). 字符串S中的最长回文
 * 方案：S + $ 与倒转的S（S’） + $ 建立后缀树，从第i个字符为回文中心点，依次检查以S[i],S'[N-i-1]开始
 * 的后缀的公共祖先的深度，最长的即为最长回文的半径（考虑偶数回文与奇数回文）
 */
