...
public class StringMatch {
   ...
   */
  static int kmp(String str, String patt) {
    int m = str.length(), n = patt.length();
    int[] next = new int[n];
    for(int i=1;i<n;i++) {
      String tmp = patt.substring(0,i+1);
      for(int j = i;j>=0;j--) {
        if(tmp.substring(0,j) == tmp.substring(i-j+1,i+1)) {
   ...
   * 最佳O（m），最差O（mn）
   */
  static int bm(String str, String patt) {
    int m = str.length(), n = patt.length();
    int i=n-1, j=n-1;
    while(i<m) {
      int tmp = i, p = j;
      for(;p>=0;p--) {
        if(str.charAt(tmp--) != patt.charAt(p)) {
          // 找到最右边匹配的下标（不包含最后一位，故为p-1）
          int index = findChar(str.charAt(i), patt, j-1);
          i += p-index;
          break;
        }
      }
   ...
   * 最佳O（m），最差O（mn）
   */
  static int sunday(String str, String patt) {
    int m = str.length(), n = patt.length();
    int i=0;
    while(i<=m-n) {
      int tmp = i, p = 0;
      for(;p<n;p++) {
        if(str.charAt(tmp++) != patt.charAt(p)) {
          // 找到最右边匹配的下标（不包含最后一位，故为p-1）
          if(i==m-n) return -1;
          int index = findChar(str.charAt(i+n), patt, n-1);
          i += n-index;
          break;
        }
      }
